...
if (-not $env:DATABASE_URL) {
    Write-Error "Set DATABASE_URL first."
}
...
$TaskJson = $TaskJson.Replace("DATABASE_URL_VALUE", ($env:DATABASE_URL -replace '\\', '\\\\' -replace '"', '\"'))
...
$TaskFile = "file://" + ($TempTask -replace '\\', '/')
aws ecs register-task-definition --cli-input-json $TaskFile --region $Region | Out-Null
if ($LASTEXITCODE -ne 0) { Write-Error "Failed to register task definition." }
...
if ($LASTEXITCODE -ne 0 -or -not $SgId -or $SgId.Trim() -eq "None") {
    ...
    $ErrorActionPreference = "Continue"
    aws ec2 authorize-security-group-ingress --group-id $SgId --protocol tcp --port 8501 --cidr 0.0.0.0/0 --region $Region 2>$null | Out-Null
    $ErrorActionPreference = "Stop"
    Write-Host "Created SG $SgId (port 8501 open). Add inbound 5432 on RDS SG from this SG."
} else {
    $SgId = $SgId.Trim()
}
...
$ErrorActionPreference = "Continue"
$Existing = (aws ecs describe-services --cluster $Cluster --services $Service --region $Region --query "services[?serviceName=='$Service'].status" --output text 2>$null)
$ErrorActionPreference = "Stop"
$serviceActive = ($LASTEXITCODE -eq 0 -and $Existing.Trim() -eq "ACTIVE")
...
if ($LASTEXITCODE -ne 0) { Write-Error "Failed to create or update ECS service." }

Write-Host ""
Write-Host "Deploy started. Wait 2-3 min, then run: .\scripts\get-ecs-url.ps1"
Write-Host "Add RDS security group inbound: PostgreSQL 5432 from $SgId"
